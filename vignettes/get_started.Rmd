...
Have a look at the default marginal model.
...
For the first example do not adjust the default ARMA-GARCH model specification. So one just needs to specify the train and refit size in order to perform the rolling window estimation.
...
Now specify the settings for the vine copula fitting. Here one can for example use only a subset of all the one-parametric bivariate copulas as the building blocks of the final fitted vine copulas.
...
Package is written with extensive input checks that prevent the user from using the functions in an unintended way. An example for this is given below.
